...
def increaseLikes(request,id):
    object = blogs.objects.filter(id=id)
    try:
        dictionaryList = request.session['likes'+str(object[0].id)]
    except:
        request.session['likes'+str(object[0].id)] = [{'user':request.user.username,'isLiked':True}]
        dictionaryList = request.session['likes'+str(object[0].id)]
    print(dictionaryList)
    for i in dictionaryList:
        if request.user.username in i['user'] and i['isLiked'] == True:
            print('alreadyLiked')
        else:
            object.update(likes=object[0].likes+1)
            request.session['likes'+str(object[0].id)].append({'user':request.user.username,'isLiked':True})
    return HttpResponse(object[0].likes)
    print("executed")
